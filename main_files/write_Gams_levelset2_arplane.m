function [coeffs,ws,exp_mat,R1,mv,l0,l0_new,mu,w,ee,c,indx1,indy1,cnum,cluster_num]= write_Gams_levelset2_arplane(im,file,fixed,weight,csize,dsize,rnum,begin)
...
%         %lbd(cnum+mon_num+2+vercount+cnum)=0;%eres
        ...
        % lbd(mon_num+cnum+1:cnum+mon_num+bnum)=0.0001;
         ...
         lbd(1:mon_num-1)=-2*5;
        % lbd(mon_num+1)=1;
        % lbd(mon_num+1:mon_num+322)=1;
         %lbd(mon_num+10)=1;
      %  lbd(mon_num+1:mon_num+467)=0.8;
          %  lbd(mon_num+1:mon_num+106)=0.999;
%      lbd(mon_num+400)=1;
%      lbd(mon_num+440)=1;
    %  lbd(mon_num+35:mon_num+50)=1;
        start_point=1
...
%         %ubd(cnum+mon_num+2+vercount+cnum+3)=0.5;
